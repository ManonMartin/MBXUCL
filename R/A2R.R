# code from Romain Francois's addicted to R graph gallery
...
"A2Rplot.hclust" <- function(
  ...
  # prepare the layout
  matlayout <- matrix(c(2,4,6,1,3,5), ncol =2, nrow=3)
  widths    <- c(1,9)
  heights   <- c(8,1,1)
  if(!show.labels){
      matlayout <- matrix(c(2,4,1,3), ncol=2, nrow=2)
      widths    <- c(1,9)
      heights   <- c(9,1)
  }
  if(!missing(fact.sup) ) {
  ...
  layout(matlayout, widths=widths, height=heights)
