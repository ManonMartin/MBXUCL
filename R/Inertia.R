...
Inertia <- function(x, y, print = FALSE){
  ...
  res1 <- matrix(data = c(InertiaB, InertiaW, InertiaT, PInertiaB, PInertiaW, PInertiaT),
                ncol = 3, dimnames = list(c("Value", "Percentage"), c("BI", "WI", "TI")),
                byrow = TRUE)
  ...
  numbers <- c(nG, n)
  res2 <- matrix(data = c(numbers, Inertia_group, Inertia_TOT_group, Inertia_group100,
                Inertia_moy_group, NA), ncol = 4, dimnames = list(c(paste0("Group ", unique(y)),
                "Total"), c("N", "Inertia_group", "Inertia_group100", "Inertia_moy_group")),
                byrow = FALSE)
